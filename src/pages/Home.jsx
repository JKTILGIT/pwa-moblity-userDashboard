    ...
    const fetchTickets = async () => {
      try {
        const res = await fetch(
          `${API_BASE}/api/zoho/tickets/mechanic/${userId}`
        );
        if (!res.ok) {
        ...
        const transformedTickets = (data.tickets || []).map((ticket) => ({
          ...
          paymentStatus:
            ...
            (ticket.status === "Completed" ? "paid" : "pending"),
        }));
    ...
    <div className="container">
      ...
      >
        ...
        <div
          style={{
            padding: "5px 10px",
            borderRadius: "100px",
            backgroundColor: `${showRequests ? "#FB8C00" : "#000"}`,
            color: "white",
          }}
        >
          <p
      ...
      {!showRequests && (
        <>
          ...
          <div className="list">
            {loading && (
              <div className="caption-text">{t("common.loading")}</div>
            )}
            {error && (
              <div style={{ color: "red" }} className="text-field">
                {error}
              </div>
            )}
            {!loading &&
            ...
            {!loading && !error && filteredTickets.length === 0 && (
              <div className="caption-text">{t("home.noTicketsFound")}</div>
            )}
          </div>
        </>
      )}
